...
function v4Format(string $input): string
{
  if ($input === '0')
...
function v6Format(string $input): string
{
  $htmlized = '';
  for ($i = 0; $i < strlen($input); $i++) {
    $char = $input[$i];
    $charClass = '';
    if ($char === '0') $charClass = 'all-zero';
    else if ($char === 'f') $charClass = 'all-one';
    $htmlized .= "<span class=\"$charClass\">$char</span>";
  }

  return "<span class=\"ip-part\">$htmlized</span>";
}
...
<section id="v4" class="<?= $v6mode ? 'hide' : '' ?>">
  ...
  <div>
    <form action="" method="get">
      ...
      <input type="text" name="v4s" id="v4s" value="<?= htmlentities($v4s) ?>" size="15" maxlength="15"
             pattern="([0-2]?[0-9]{1,2}\.){3}[0-2]?[0-9]{1,2}">
